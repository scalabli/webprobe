...
from webprobe.lib.utils.file import FileUtils
...
class ArgumentParser:
    ...
    def access_file(
        ...
        with File(path) as file:
            if not file.exists():
                ...
                echo(f"The {name} does not exist", fg="red", bold=True)
                exit(1)
    ...
    def parse_config(self):
        ...
        # Reports
        # if not set reports and logs output directory,
        ...
        logsPath = homePath+"/.webprobe/logs"
        if not os.path.isdir(logsPath):
            os.makedirs(logsPath)
        self.output_location = config.safe_get("reports", "report-output-folder", reportsPath)
        self.autosave_report = config.safe_getboolean("reports", "autosave-report", False)
        self.logs_location = config.safe_get("reports", "logs-location", logsPath)
    ...
    def parse_arguments(self):
        ...
        mandatory.add_option("-f", "--force-extensions", action="store_true", dest="force_extensions", default=self.force_extensions,
                             help="Add extensions to every wordlist entry. By default webprobe only replaces the %EXT% keyword with extensions")
        ...
        connection.add_option("-b", "--request-by-hostname",
                              help="By default webprobe requests by IP for speed. This will force webprobe to request by hostname",
